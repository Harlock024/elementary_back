from django.db import transaction
from django.db.models import F

from academics.models import ClassRoom
from assignments.application.dtos.assignments_dto import CreateAssignmentCommand, UpdateAssignmentCommand
from assignments.domain.exceptions.assignments_exception import (
    AssignmentsAlreadyExistsError,
    AssignmentsNotFoundError,
    ClassRoomNotFoundError,
)
from assignments.models import Assignment
from grades.models import GradingCriteria


class DjangoAssignmentRepository:
    def list_assignments(
        self,
        class_id: str | None = None,
        student_id: str | None = None,
        due_date=None,
        classroom_ids: list[str] | None = None,
    ) -> list[dict]:
        qs = Assignment.objects.select_related("class_room", "subject", "grading_criteria")
        if class_id:
            qs = qs.filter(class_room_id=class_id)
        if student_id:
            qs = qs.filter(
                class_room__group__enrollments__student_id=student_id,
                class_room__group__enrollments__academic_period_id=F("class_room__academic_period_id"),
            ).distinct()
        if due_date:
            qs = qs.filter(due_date=due_date)
        if classroom_ids is not None:
            qs = qs.filter(class_room_id__in=classroom_ids)
        return [self._to_dict(a) for a in qs]

    def create_assignment(self, command: CreateAssignmentCommand) -> dict:
        with transaction.atomic():
            classroom = ClassRoom.objects.select_for_update().select_related(
                "academic_period", "group"
            ).filter(pk=command.class_id).first()
            if classroom is None:
                raise ClassRoomNotFoundError("ClassRoom not found")
            if classroom.academic_period.status == "closed":
                raise ValueError("The academic period is closed")
            criteria = GradingCriteria.objects.select_for_update().filter(
                pk=command.grading_criteria_id,
                class_room=classroom,
            ).first()
            if criteria is None:
                raise AssignmentsAlreadyExistsError("GradingCriteria not found in classroom")
            if not classroom.group.school_grade.subjects.filter(pk=command.subject_id).exists():
                raise AssignmentsAlreadyExistsError("Subject does not belong to classroom grade")
            due_date = command.due_date.date() if hasattr(command.due_date, "date") else command.due_date
            if not classroom.academic_period.start_date <= due_date <= classroom.academic_period.end_date:
                raise ValueError("Assignment date is outside the academic period")
            assignment = Assignment.objects.create(
                class_room=classroom,
                subject_id=command.subject_id,
                grading_criteria=criteria,
                title=command.title,
                description=command.description,
                due_date=due_date,
                max_score=command.max_score,
            )
        assignment = Assignment.objects.select_related("subject", "grading_criteria").get(pk=assignment.pk)
        return self._to_dict(assignment)

    def update_assignment(self, command: UpdateAssignmentCommand) -> dict:
        with transaction.atomic():
            assignment = Assignment.objects.select_for_update().select_related(
                "subject", "grading_criteria", "class_room__academic_period"
            ).filter(pk=command.assignment_id).first()
            if assignment is None:
                raise AssignmentsNotFoundError("Assignment not found")
            if assignment.class_room.academic_period.status == "closed":
                raise ValueError("The academic period is closed")
            if command.title is not None:
                assignment.title = command.title
            if command.description is not None:
                assignment.description = command.description
            if command.due_date is not None:
                due_date = command.due_date.date() if hasattr(command.due_date, "date") else command.due_date
                if not assignment.class_room.academic_period.start_date <= due_date <= assignment.class_room.academic_period.end_date:
                    raise ValueError("Assignment date is outside the academic period")
                assignment.due_date = due_date
            if command.max_score is not None:
                assignment.max_score = command.max_score
            if command.subject_id is not None:
                if not assignment.class_room.group.school_grade.subjects.filter(pk=command.subject_id).exists():
                    raise ValueError("Subject does not belong to classroom grade")
                assignment.subject_id = command.subject_id
            if command.grading_criteria_id is not None:
                valid = GradingCriteria.objects.select_for_update().filter(
                    pk=command.grading_criteria_id,
                    class_room_id=assignment.class_room_id,
                ).exists()
                if not valid:
                    raise ValueError("Grading criteria must belong to classroom")
                assignment.grading_criteria_id = command.grading_criteria_id
            assignment.save()
        return self._to_dict(assignment)

    def delete_assignment(self, assignment_id: str) -> None:
        assignment = Assignment.objects.select_related("class_room__academic_period").filter(pk=assignment_id).first()
        if assignment is None:
            raise AssignmentsNotFoundError("Assignment not found")
        if assignment.class_room.academic_period.status == "closed":
            raise ValueError("The academic period is closed")
        assignment.delete()

    def _to_dict(self, assignment: Assignment) -> dict:
        subject = assignment.subject
        grading_criteria = assignment.grading_criteria
        return {
            "id": str(assignment.id),
            "class_room_id": str(assignment.class_room_id),
            "subject": {
                "id": str(subject.id),
                "name": subject.name,
            } if subject else None,
            "grading_criteria_id": str(assignment.grading_criteria_id),
            "grading_criteria": {
                "id": str(grading_criteria.id),
                "name": grading_criteria.name,
                "percentage": float(grading_criteria.percentage),
            } if grading_criteria else None,
            "title": assignment.title,
            "description": assignment.description,
            "due_date": str(assignment.due_date),
            "max_score": float(assignment.max_score),
            "created_at": assignment.created_at.isoformat(),
            "updated_at": assignment.updated_at.isoformat(),
        }
